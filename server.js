...
app.get("/playlist-progress", async (req, res) => {
    ...
    try {
        ...
        if (esSpotify) {
            ...
            const rawIds = await execPromise(`yt-dlp --js-runtimes /usr/local/bin/node --extractor-args youtube:player_client=web --get-id --flat-playlist "${url}"`);
        ...
        for (let i = 0; i < total; i++) {
            ...
            const comando = esSpotify
                ? `yt-dlp --js-runtimes /usr/local/bin/node --extractor-args youtube:player_client=web -x --audio-format mp3 --no-playlist -o "${folderPath}/%(title)s.%(ext)s" "ytsearch1:${cancion}"`
                : `yt-dlp --js-runtimes /usr/local/bin/node --extractor-args youtube:player_client=web -x --audio-format mp3 --no-playlist -o "${folderPath}/%(title)s.%(ext)s" "${cancion}"`;
