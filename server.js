    ...
    try {
        ...
        for (let i = 0; i < total; i++) {
            ...
            const comando = esSpotify
                ? `yt-dlp --js-runtimes node --extractor-args youtube:player_client=web -x --audio-format mp3 --no-playlist -o \"${folderPath}/%(title)s.%(ext)s\" \"ytsearch1:${cancion}\"`
                : `yt-dlp --js-runtimes node --extractor-args youtube:player_client=web -x --audio-format mp3 --no-playlist -o \"${folderPath}/%(title)s.%(ext)s\" \"${cancion}\"`;
